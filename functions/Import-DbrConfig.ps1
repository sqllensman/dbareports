function Import-DbrConfig {
    <#
        .SYNOPSIS
            Imports dbareports configs from a json file
    ...
    process {
        if (-not (Test-Path -Path $Path)) {
            Stop-PSFFunction -Message "$Path does not exist. Run Export-DbrConfig to create."
            return
        }
        ...
        foreach ($result in $results) {
            Set-DbrConfig -Name $result.Name -Value $result.Value -Temporary:$Temporary
        }
    }
}