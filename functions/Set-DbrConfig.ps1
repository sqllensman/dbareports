function Set-DbrConfig {
    <#
        ...
        .EXAMPLE
            Set-DbrConfig -Name app.sqlinstance -Value sql2016
        
            Resets the app.sqlinstance entry to sql2016
    
        .EXAMPLE
            Set-DbrConfig -Name Lists.SqlServers -Value sql2016, sql2017, sqlcluster -Append
    ...
    process {
        if (-not (Get-DbrConfig -Name $Name)) {
            Stop-PSFFunction -Message "Setting named $Name does not exist. If you'd like us to support an additional setting, please file a GitHub issue." -Tag $Module
            return
        }
        
        if ($append) {
            $Value = (Get-DbrConfigValue -Name $Name), $Value
        }
        
        $Name = $Name.ToLower()
        
        Set-PSFConfig -Module $Module -Name $name -Value $Value
        try {
            if (-not $Temporary) { Register-PSFConfig -FullName $Module.$name -EnableException -WarningAction SilentlyContinue }
        }
        catch {
            Set-PSFConfig -Module dbareports -Name $name -Value ($Value -join ", ")
            if (-not $Temporary) { Register-PSFConfig -FullName $Module.$name }
        }
